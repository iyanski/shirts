...
if(is_file('app/views/' . $class_name[0] . '/' . $action . '.php')){
	...
	require_once (is_file($t_file = 'public/skins/' . $config['THEME_PATH'] . '/templates/header.php')) ? $t_file : '';
	require_once ('app/views/' . $class_name[0] . '/' . $action . '.php');
	require_once (is_file($t_file = 'public/skins/' . $config['THEME_PATH'] . '/templates/footer.php')) ? $t_file : '';
}
#end
?>